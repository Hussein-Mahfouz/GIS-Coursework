...
Plot to see if the roads have been downloaded. This takes around 10 minutes and the sp file is huge.
It can be skipped 
...
Now we're done with otp. Make sure to run otp_stop() followed by ENTER to terminate Java (it takes up 2GB of precious memory)
...
as.numeric((cairo_hexagons$jobs_60_formal %*% cairo_hexagons$pop2018cap) / sum(cairo_hexagons$pop2018cap))
# # Average accessibility using formal Modes (%):
as.numeric((cairo_hexagons$access_per_60_formal %*% cairo_hexagons$pop2018cap) / sum(cairo_hexagons$pop2018cap))
